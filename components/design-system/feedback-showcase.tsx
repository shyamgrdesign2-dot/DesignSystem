...
function TPCloseIcon({ size = 20, color = "white" }: { size?: number; color?: string }) {
  return (
...
function ToastBar({
  ...
  return (
    ...
    >
      ...
      {action === "close" ? (
        <button
          className="flex-shrink-0 inline-flex items-center justify-center hover:opacity-70 transition-opacity"
          style={{ color: variant.closeColor }}
        >
          <TPCloseIcon size={20} color={variant.closeColor} />
        </button>
      ) : (
        <button
